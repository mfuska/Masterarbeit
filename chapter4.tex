...
\begin{figure}[hp!]
        ...
        \caption{iOS Software Layer (Quelle \cite{Apple[5]} S. 9)}
...
\label{sec:CoreLayer}
Der \textbf{Core OS Layer} beinhaltet alle \textit{\glqq Low Level Features\grqq}. Auf diesem Layer bauen alle anderen \textit{\glqq Frameworks\grqq{}} auf. Der iOS Applikationsentwickler kommt mit diesem Layer nur dann in Berührung, wenn dieser sich mit der Sicherheit und Hardware-Kommunikation beschäftigt. 
\begin{description}
...
\begin{figure}[htb]
  ...
  Der Sicherheitsmechanismus des iOS Device unterteilt sich in zwei Ebenen. 
  		\begin{description}
			...
			\end{enumerate}
   		\end{description}
Die iOS Sicherheit basiert auf einer Kombination aus Software, Hardware und Services, welche ein Maximum an Sicherheit garantieren soll (vgl. \cite{Apple[4]} S.4).
	\end{minipage}
	\hfil
